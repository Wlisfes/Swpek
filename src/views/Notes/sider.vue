 ...
 * @LastEditors: 情雨随风
 * @LastEditTime: 2019-08-05 22:32:16
 * @Description: 侧边栏标签组件
 -->
        ...
        >
            <div class="content">
                <div class="siderwrapper-content">
                    ...
                    <ul>
                        <li class="li-tags" @click="AsNotes">
...
export default {
    ...
    methods: {
        ...
        async Assider() {
            try {
                await this.$store.dispatch('notes/Assider')
            } catch (error) {}
        },
        //选择某个标签
        async handelitemID(item) {
            this.$store.commit('notes/setcheckSider', item)
            await this.$store.dispatch('notes/AsNotesID')
        },
        //文章全部列表
        async AsNotes() {
            try {
                await this.$store.dispatch('notes/AsNotes', 500)
            } catch (error) {}
        }
    }
}
